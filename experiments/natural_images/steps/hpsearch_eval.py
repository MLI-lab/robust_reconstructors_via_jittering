...
def run_hpsearch_eval(
    ...
    if evaluation:
        ...
        # build up test datasets
        #valdir = os.path.join(d_set['path'], 'val', d_set["tensorlist_name"])
        #dataset_per_device = {device:TensorListDataset(valdir, device=device) for device in devices}
    ...
    if plot:
        ...
        plot_step(base_path_eval, zetas, alphas, sig_energy, noise_level, method_name,
            data_last_min, data_last_mean, data_last_median, data_last_std, "last",conv_type)
        plot_step(base_path_eval, zetas, alphas, sig_energy, noise_level, method_name,
            data_min_min, data_min_mean, data_min_median, data_min_std, "min", conv_type)

def plot_step(base_path_jit_unet_eval, zetas, alphas, sig_energy, noise_level, method_name, data_min, data_mean, data_median, data_std, label, conv_type):
    # full plot
    ...
    if method_name == "jittering" and conv_type == "None":
        n = 128*128*3
        d = n / 2
